...
const COMMAND_ERROR_SUBTYPES = {
  warn: 'échec_avertissement',
  kick: 'échec_expulsion',
  ban: 'échec_bannissement',
  unban: 'échec_débannissement',
  timeout: 'échec_timeout',
  untimeout: 'échec_retrait_timeout',
  warnings: 'échec_consultation_avertissements',
  ticket: 'échec_ticket',
  serverstats: 'échec_stats_serveur',
  gcreate: 'échec_giveaway',
  gend: 'échec_giveaway',
  gdelete: 'échec_giveaway',
  greroll: 'échec_giveaway',
};
    ...
    return runWithTraceContext(interactionTraceContext, async () => {
      try {
        ...
        if (interaction.isChatInputCommand()) {
          try {
            ...
            if (isMaintenanceMode() && !isBotOwner(interaction.user.id)) {
              throw createError(
                'Le bot est en mode maintenance',
                ErrorTypes.CONFIGURATION,
            ...
            if (!isCommandCategoryEnabled(command.category)) {
              throw createError(
                `Fonctionnalité désactivée pour la catégorie ${command.category}`,
                ErrorTypes.CONFIGURATION,
            ...
            if (defaultCooldownSec > 0 && !isBotOwner(interaction.user.id)) {
              ...
              if (expiresAt && Date.now() < expiresAt) {
                ...
                throw createError(
                  `Le délai d'attente de la commande ${interaction.commandName} est actif`,
            ...
            if (!abuseProtection.allowed) {
              ...
              throw createError(
                `Le délai d'attente de la commande ${interaction.commandName} est actif`,
                ErrorTypes.RATE_LIMIT,
                `Cette commande est temporairement indisponible. Veuillez patienter ${formattedCooldown} avant de réessayer.`,
                withTraceContext({
                  commandName: interaction.commandName,
            ...
            if (interaction.guild) {
              ...
              if (
                ...
                throw createError(
                  `La commande ${accessKey} est désactivée sur ce serveur`,
                  ErrorTypes.CONFIGURATION,
                  'Cette commande a été désactivée sur ce serveur.',
                  withTraceContext({
          ...
          if (autocompleteCommand?.autocomplete) {
            try {
              ...
              logger.error('Erreur lors de la gestion de l’autocomplétion de la commande :', {
                error: error.message,
                guildId: interaction.guildId,
                commandName: interaction.commandName,
              });
          ...
          if (
            ...
            try {
              ...
              await interaction.respond(
                filtered.slice(0, 25).map(role => ({
                  name: `${role.name}${role.enabled === false ? ' (désactivé)' : ''}`,
                  value: role.name
                }))
              );

            } catch (error) {
              logger.error('Erreur lors de la gestion de l’autocomplétion :', {
                error: error.message,
                guildId: interaction.guildId,
                commandName: interaction.commandName
              });
            ...
            try {
              ...
              logger.error('Erreur lors de l’autocomplétion de app-admin :', {
                error: error.message,
                guildId: interaction.guildId,
                commandName: interaction.commandName
              });
            ...
            try {
              ...
              let panels = await getAllReactionRoleMessages(
                client,
                guildId
              );
              ...
              logger.error(
                'Erreur lors de l’autocomplétion de reactroles :',
                {
                  error: error.message,
                  guildId: interaction.guildId,
                  commandName: interaction.commandName
          ...
          if (interaction.customId.startsWith('shared_todo_')) {
            ...
            if (button) {
              ...
              throw createError(
                `Aucun gestionnaire de bouton trouvé pour ${buttonType}`,
                ErrorTypes.CONFIGURATION,
          ...
          if (!button) {
            ...
            throw createError(
              `Aucun gestionnaire de bouton trouvé pour ${customId}`,
              ErrorTypes.CONFIGURATION,
              'Ce bouton n’est pas disponible.',
              withTraceContext(
                { customId },
          ...
          if (!selectMenu) {
            ...
            throw createError(
              `Aucun gestionnaire de menu déroulant trouvé pour ${customId}`,
              ErrorTypes.CONFIGURATION,
              'Ce menu déroulant n’est pas disponible.',
              withTraceContext(
                { customId },
                interactionTraceContext
              )
          ...
          if (
            ...
            logger.debug(
              `Gestionnaire de modal ignoré pour la modal intégrée : ${interaction.customId}`,
              {
          ...
          if (!modal) {
            ...
            throw createError(
              `Aucun gestionnaire de formulaire trouvé pour ${customId}`,
              ErrorTypes.CONFIGURATION,
        ...
        try {
          ...
          logger.error(
            'Échec de l’envoi de la réponse d’erreur de secours :',
            {
